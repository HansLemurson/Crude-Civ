...
class Tile:
  ...
  def __init__(self,grid,terrain_type,col,row):
    ...
    self.row = row
    self.grid = grid
    self.terrain = terrain.getTerrain(terrain_type)
    self.color = Tile.terrain_colors[terrain_type]
    self.units = []
    self.city = None
  #----------------------------------------------------------
  def __str__(self):
...
class Grid:
  ...
  def tryToMoveUnitTo(self,target_tile):
    '''If there is a unit to move, and a valid place to move it to, do so.'''
    pass
  #----------------------------------------------------------
  def hasMouse(self):
      ...
      # Handle Right-Clicks
        ...
        if self.selected_coords: #If a tile is even selected
          ...
          if selected_tile.units: #If that tile has any units
            active_unit = selected_tile.units[0] #Take first unit in list
...
world.addUnit("Warrior",7,5)
world.addUnit("Warrior",1,2)
